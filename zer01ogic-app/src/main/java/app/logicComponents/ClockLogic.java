...
public class ClockLogic implements ILogicElement {
    private final ClockGraphic parentGraphic;
    private IObservableValue<Integer> output;

    public ClockLogic(ClockGraphic parentGraphic) {
...
//        if(index!=0) throw
    ...
    public void clockState() {
        output.setValue(~output.getValue());
        if (output.getValue() != 0)
            parentGraphic.setStyle(ClockGraphic.ClockStyle.On);
        else
            parentGraphic.setStyle(ClockGraphic.ClockStyle.Off);
    }
}
